...
func buildRequestFromHttpGateway(event *HttpAPIGatewayProxyRequest) (*http.Request, error) {
	...
	req, err := http.NewRequest(event.RequestContext.HTTP.Method, u.String(), strings.NewReader(rawBody))
	if err != nil {
...
func HttpGatewayProxyEvent(handler http.Handler) func(ctx context.Context, event HttpAPIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	return func(ctx context.Context, event HttpAPIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		req, err := buildRequestFromHttpGateway(&event)
		if err != nil {
